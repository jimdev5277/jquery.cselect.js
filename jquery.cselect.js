...
(function ($) {
    function clearOptionMethod(select_showbox, ul_option) { //清理弹出后其他元素的处理
        ...
        }).data('seldefaultindex', 0).children('li').css('padding-right', ul_option[0].offsetWidth - ul_option[0].scrollWidth);//滚动条留白
    ...
    function createSelect_cSelect(select_container, index, singleClassName, options) {
        ...
        $(document).bind("keyup", function (e) {//添加按下esc收起下拉列表
            ...
            if (keyCode == 27) {
                ...
                if (ul_option.css("display") != 'none') {
                    var seldefaultindex = ul_option.data('seldefaultindex') + 1;
                ...
                if (options.autocomplete) {
                    ul_option.prev('input').data('enter-operation',true);
                    console.info('enter-operation', ul_option.prev('input').data('enter-operation'));
                    ul_option.prev('input').blur();
                }
            }
